...
ttk.Button(button_frame, text="Run Progressbar", command=run_progressbar).grid(row=0, )
ttk.Button(button_frame, text="Start Progressbar", command=start_progressbar).grid(row=1, )
ttk.Button(button_frame, text="Stop immediately", command=stop_progressbar).grid(row=2, )
ttk.Button(button_frame, text="Stop after second", command=stop_after_progressbar).grid(row=3, )
...
tab3_frame = tk.Frame(
    tab3,
    background='blue'  # only work for tk not ttk,
    # see https://docs.python.org/3.6/library/tkinter.ttk.html#ttk-widgets
)
tab3_frame.pack()
...
for orange_color in range(2):
    ...
    canvas.grid(
        row=orange_color, column=orange_color,
    )

spin_data = spin.get()
print('Spinbox value:', spin_data)
